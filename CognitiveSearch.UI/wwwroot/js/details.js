    ...
    $.post('/home/getdocumentbyid',
        ...
        function (data) {
            ...
            if (transcriptContainerHTML !== null) {
                ...
                pivotsHTML += '          <div id="file-pivot" class="ms-Pivot-content" data-content="file"> ';
            ...
            if (transcriptContainerHTML !== null) {
                pivotLinksHTML += '<li id="transcript-pivot-link" class="ms-Pivot-link is-selected" data-content="transcript" title="Transcript" tabindex="1">Transcript</li>';
                pivotLinksHTML += '<li id="file-pivot-link" class="ms-Pivot-link" data-content="file" title="Raw File" tabindex="1">' + fileName + '</li>';
            }
            else {
                pivotLinksHTML += '<li id="file-pivot-link" class="ms-Pivot-link is-selected" data-content="file" title="Raw File" tabindex="1">' + fileName + '</li>';
            }
...
function GetMetadataHTML(result) {
    ...
    for (var key in result) {
        if (result.hasOwnProperty(key) || result[key] === "web_archive_org_link" ) {
            if (key !== "content" &&  key !== "enriched" && key !== "id" && key !== "layoutText" && key !== "ImageTags" && key !== "ImageCaption" && key !== "text" && key !== "merged_content" && key !== "translated_text" && key !== "keyphrases") {
                if (result[key] !== null) {
                    ...
                    if (key === "metadata_storage_path") {
                        ...
                        if (value.includes("/websites/")) {
                            // This is a web.archive.org result that we will try to map to a Wayback Machine URL:
                            var rawPath = value.replace("https://eqarchives.blob.core.windows.net/eq-archives/websites/", "");
                            if (rawPath.match("[0-9]{14}")) {
                                var dateString = rawPath.match("[0-9]{14}")[0];
                                var cleansedPath = rawPath.replace("/" + dateString, "");
                                var constructedUrl = "https://web.archive.org/" + dateString + "/" + cleansedPath

                                // special handling for eq.castersrealm.com links, because I was an idiot and ran 'detox' on its capture files, and it mangled the query strings
                        ...
                        if (value.includes("/mailing-lists/")) {
                            // This is a mailing_list result that we will map to a Yahoo Groups mailing list:
                            var rawPath = value.replace("https://eqarchives.blob.core.windows.net/eq-archives/mailing-lists/", "");
                            var listName = rawPath.split("/")[0];
                            metadataContainerHTML += '<tr><td class="key"  style="width:50%" >' + "yahoo_groups_email_list_name" + '</td><td class="wrapword"  style="width:50%" >' + listName + '</td></tr>';
                        }
                    }
